pipeline {
    ...
    tools {
        maven 'Maven3.8' // Для сборки бэкенда нужен Maven
        jdk 'JDK16' // И Java Developer Kit нужной версии
        nodejs '16.0.0' // А NodeJS нужен для фронта
    }
    ...
    stages {
        ...
        stage('Notify slack') {
            environment {
                DVS_SLACK_TOKEN = credentials('DVS_SLACK_TOKEN')
            }
            steps {
                sh '''curl -X POST -H \'Content-type: application/json\' --data \'{"text":"Евгений Попов собрал приложение."}\' \
                 https://hooks.slack.com/services/TPV9DP0N4/B040ZBELF63/\$DVS_SLACK_TOKEN'''
            }
        }
    }
}